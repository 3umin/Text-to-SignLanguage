...
os.chdir('C:/Users/ytnal/.jupyter/Github/mc')
filelist = os.listdir()

for filename in filelist:
    file = filename[:-4]
    filecopy = file + '.001'
    filelink = "C:\\Users\\ytnal\\.jupyter\\Github\\mc\\" + file + '.bvh'
    bpy.ops.import_anim.bvh(filepath=filelink)
    bpy.ops.import_anim.bvh(filepath=filelink)

    ########## Variable declarations ##########

    ## Name of imported BVH and its copy
    arm_title = file # x-axis will be rotated to -210d
    arm_title_head = filecopy # x-axis will be rotated to -180d to map the neck and head bones
    ...
    bpy.context.scene.render.filepath = 'C:\\tmp\\' + file

    bpy.ops.render.render(animation=True)

    bpy.data.objects.remove(bpy.data.objects[file])
    bpy.data.objects.remove(bpy.data.objects[filecopy])
